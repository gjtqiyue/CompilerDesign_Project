...
struct EXP {
	...
	union {
		...
		struct { EXP *exp; } paren;
		struct { char* identifier; SYMBOL *sym; } identifier;
		struct { char* stringLiteral; } stringLiteral;
		struct { int intLiteral; } intLiteral;
		struct { float floatLiteral; } floatLiteral;
		struct { int boolLiteral; } boolLiteral;
	} val;
};
