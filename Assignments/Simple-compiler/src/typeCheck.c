...
TYPE* typeEXP(EXP *e) {
    ...
    switch (e->kind) {
        ...
        case k_ExpressionKindAdd:
            ...
            TYPE* res = compareArithmetic(lhs, rhs, e->lineno);
            if (res != NULL) {
                e->type = res;
                return e->type;
            }
        ...
        case k_ExpressionKindDivide:
            lhs = typeEXP(e->val.binary.lhs);
            rhs = typeEXP(e->val.binary.rhs);
            checkArithmetic(lhs, e->lineno);
            checkArithmetic(rhs, e->lineno);
            if (lhs->kind == rhs->kind && lhs->kind == k_TypeKindInt) {
                t = malloc(sizeof(TYPE));
                t->kind = k_TypeKindInt;
                t->lineno = e->lineno;
                e->type = t;
        ...
        case k_ExpressionKindNotEqual:
            lhs = typeEXP(e->val.binary.lhs);
            rhs = typeEXP(e->val.binary.rhs);
            e->type = checkBinaryComparison(lhs, rhs, e->lineno);    
            //return boolean
            return e->type;

        case k_ExpressionKindUMinus:
            e->type = typeEXP(e->val.unary.exp);
            checkArithmetic(e->type, e->lineno);
            return e->type;
    }
}
...
void typePROG(PROG *p) {
    ...
    case k_StatementKindWhile:
        checkBOOL(typeEXP(p->val.while_stmt.cond), p->lineno);
        typePROG(p->val.while_stmt.body);
        break;
    case k_StatementKindIf:
        checkBOOL(typeEXP(p->val.if_stmt.cond), p->lineno);
    ...
    case k_StatementKindDeclarationType:
        ...
        //update the symbol stored in the AST
        checkAssign(p->val.decl_type.name->val.identifier.sym->type, type, p->lineno);
        break;
    }
}
...
TYPE* checkAssign(TYPE *lhs, TYPE *rhs, int lineno) {
    if (lhs->kind == rhs->kind || (lhs->kind == k_TypeKindFloat && rhs->kind == k_TypeKindInt)) {
        return lhs;
    }
    else {
        fprintf(stderr, "Error: line(%d) the assignment has different types [%s, %s]\n", lineno, printType(lhs), printType(rhs));
        exit(1);
    }
}
...
TYPE* compareArithmetic(TYPE *lhs, TYPE *rhs, int lineno) {
    ...
    else if ((lhs->kind == k_TypeKindFloat && rhs->kind == k_TypeKindInt) || (rhs->kind == k_TypeKindFloat && lhs->kind == k_TypeKindInt)){
        //return float type
